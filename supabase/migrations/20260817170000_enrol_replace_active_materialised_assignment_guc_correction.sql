-- Allow the authorised catalogue-enrolment RPC to supersede a materialised
-- active assignment without relaxing the materialisation integrity trigger.
...
BEGIN
  ...
  IF FOUND THEN
    IF v_active.programme_version_id = p_programme_version_id THEN
      RETURN jsonb_build_object(
        'status', 'already_enrolled', 'enrolment_id', v_active.id,
        'programme_version_id', v_active.programme_version_id,
        'lineage_code', v_active.lineage_code,
        'enrolment_source', v_active.enrolment_source, 'athlete_id', v_athlete_id
      );
    END IF;
    IF NOT COALESCE(p_replace_active, FALSE) THEN
      RETURN jsonb_build_object(
        'status', 'conflict', 'code', 'active_enrolment_exists',
        'enrolment_id', v_active.id, 'programme_version_id', v_active.programme_version_id
      );
    END IF;
    v_replaced_id := v_active.id;
  END IF;

  -- Complete all target validation before changing the old active assignment.
  ...
  IF v_replaced_id IS NOT NULL THEN
    -- The SECURITY DEFINER role plus the trigger role check make this internal;
    -- `true` scopes it to this transaction only.
  ...
  INSERT INTO public.programme_assignments (
    ...
    v_new_id, v_athlete_id, p_programme_version_id, v_lineage_code, 'active', CURRENT_DATE,
  ...
  RETURN jsonb_build_object(
    'status', 'enrolled', 'enrolment_id', v_new_id,
    'programme_version_id', p_programme_version_id, 'lineage_code', v_lineage_code,
    'enrolment_source', 'non_commercial_test', 'athlete_id', v_athlete_id,
...
COMMENT ON FUNCTION public.enrol_athlete_in_catalogue_programme_version(UUID, TEXT, BOOLEAN) IS
  'Authenticated catalogue enrolment pins one exact eligible version. An authorised replacement retains the old assignment as reassigned and enters the privileged materialisation-write boundary only for that transaction-local lifecycle transition.';
