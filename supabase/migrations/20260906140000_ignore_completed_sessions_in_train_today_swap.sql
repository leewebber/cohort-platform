-- Product completion leaves training_sessions.status = in_progress after the
-- slot outcome is completed. Train today must treat only an open outcome as
-- an in-progress session, or lawful W1–W4 history blocks a clean +5 day swap.
