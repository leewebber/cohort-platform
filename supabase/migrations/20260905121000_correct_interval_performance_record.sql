-- Extends completed-performance correction for per-interval results.
...
BEGIN
  ...
  LOOP
    ...
    IF v_block ? 'result_data' THEN
      ...
      IF v_result ? 'averageHeartRate' THEN
        ...
        END IF;
      END IF;
      IF COALESCE(v_result->>'resultType', '') = 'interval'
         OR v_existing_block.result_type = 'interval' THEN
        v_result := public.cohort_canonicalize_interval_result_data(v_result);
      ELSIF v_result ? 'intervalsCompleted' THEN
  ...
  LOOP
    ...
    IF v_set ? 'distance' THEN
      v_distance := NULLIF(v_set->>'distance', '')::NUMERIC;
      IF v_distance IS NOT NULL AND (v_distance <= 0 OR v_distance > 1000) THEN
        RAISE EXCEPTION 'invalid_distance' USING ERRCODE = '22023';
      END IF;
    END IF;
    IF v_set ? 'duration_seconds' THEN
      v_duration := NULLIF(v_set->>'duration_seconds', '')::INT;
      IF v_duration IS NOT NULL AND (v_duration <= 0 OR v_duration > 86400) THEN
        RAISE EXCEPTION 'invalid_duration' USING ERRCODE = '22023';
      END IF;
    END IF;
    IF v_set ? 'distance_unit'
       AND NULLIF(v_set->>'distance_unit', '') IS NOT NULL
       AND lower(v_set->>'distance_unit') NOT IN ('km', 'mi', 'm') THEN
      RAISE EXCEPTION 'invalid_distance_unit' USING ERRCODE = '22023';
    END IF;
    IF v_set ? 'note'
       AND NULLIF(v_set->>'note', '') IS NOT NULL
       AND v_set->>'note' NOT IN ('pace_unavailable', 'skipped') THEN
    ...
    IF v_reps IS DISTINCT FROM v_existing_set.reps
       ...
       OR (v_set ? 'note'
           AND NULLIF(v_set->>'note', '') IS DISTINCT FROM v_existing_set.note)
       OR (v_set ? 'distance'
           AND v_distance IS DISTINCT FROM v_existing_set.distance)
       OR (v_set ? 'duration_seconds'
           AND v_duration IS DISTINCT FROM v_existing_set.duration_seconds) THEN
      IF v_set ? 'rpe' THEN
      ...
      SET reps = v_reps,
          ...
          END,
          distance = CASE
            WHEN v_set ? 'distance' THEN v_distance
            ELSE distance
          END,
          distance_unit = CASE
            WHEN v_set ? 'distance_unit' THEN NULLIF(v_set->>'distance_unit', '')
            ELSE distance_unit
          END,
          duration_seconds = CASE
            WHEN v_set ? 'duration_seconds' THEN v_duration
