...
BEGIN
 ...
 SELECT count(*) INTO v_count FROM session_blocks WHERE (session_id='APOLLO-W4-TUE-R1' AND timer_config @> '{"duration_seconds":2700,"tracking":["distance","average_pace","average_hr"]}'::jsonb) OR (session_id='APOLLO-W4-SUN-R1' AND timer_config @> '{"duration_seconds":3600,"fast_finish":false}'::jsonb);
 PERFORM sprint12_assert_eq('W','zone2_reductions_and_tracking','2',v_count::text);
 SELECT count(*) INTO v_count FROM session_block_exercises e JOIN session_blocks b ON b.block_id=e.block_id WHERE (b.session_id='APOLLO-W4-MON-R1' AND b.block_type='strength' AND ((e.position IN (1,2,5) AND e.prescription @> '{"sets":3}'::jsonb) OR (e.position IN (3,4,6,7) AND e.prescription @> '{"sets":2}'::jsonb) OR (e.position IN (8,9,10) AND e.prescription @> '{"sets":1,"intensity":"light"}'::jsonb))) OR (b.session_id='APOLLO-W4-WED-R1' AND b.block_type='strength' AND ((e.position IN (1,2) AND e.prescription @> '{"sets":3}'::jsonb) OR (e.position IN (3,4,5,6,7) AND e.prescription @> '{"sets":2}'::jsonb))) OR (b.session_id='APOLLO-W4-FRI-R1' AND b.block_type='strength' AND ((e.position=5 AND e.prescription @> '{"sets":3}'::jsonb) OR (e.position=8 AND e.prescription @> '{"sets":1}'::jsonb) OR (e.position IN (1,2,3,4,6,7,9) AND e.prescription @> '{"sets":2}'::jsonb) OR (e.position IN (10,11,12) AND e.prescription @> '{"sets":1,"intensity":"light"}'::jsonb))) OR (b.session_id='APOLLO-W4-SAT-R1' AND b.block_type='strength' AND ((e.position=1 AND e.prescription @> '{"sets":3}'::jsonb) OR (e.position IN (2,3,4,5) AND e.prescription @> '{"sets":2}'::jsonb))) OR (b.session_id='APOLLO-W4-TUE-R1' AND b.block_type='core' AND e.prescription @> '{"sets":2,"reps":"8-12"}'::jsonb);
 ...
 PERFORM sprint12_assert_eq('W','week1_week3_exercise_row_counts_unchanged','246',v_count::text);
 BEGIN
