...
BEGIN
 ...
 SELECT count(*) INTO v_count FROM session_blocks WHERE session_id='APOLLO-W5-SAT-R1' AND timer_config @> '{"rounds":3,"between_round_recovery_seconds":90,"target_rpe":"7-8","round_sequence":["EX-131","EX-132","EX-050","EX-009"],"capture_strategy":"fixed_work"}'::jsonb;
 PERFORM sprint12_assert_eq('X','three_round_conditioning_timer_fidelity','1',v_count::text);
 SELECT count(*) INTO v_count FROM session_block_exercises e JOIN session_blocks b ON b.block_id=e.block_id WHERE b.session_id='APOLLO-W5-SAT-R1' AND b.block_type='conditioning' AND e.exercise_id IN ('EX-131','EX-132') AND e.prescription @> '{"load":{"type":"athleteSelected"}}'::jsonb;
 PERFORM sprint12_assert_eq('X','sled_stations_author_external_load','2',v_count::text);
