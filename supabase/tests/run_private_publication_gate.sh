#!/usr/bin/env bash
# Dedicated local disposable gate for private publication / activation.
...
run_bc() {
  ...
  docker cp "${TESTS_DIR}/sql/gate_bc_private_publication_and_activation.sql" \
    "${SPRINT12_DB_CONTAINER}:/tmp/sprint12_gate_bc.sql"
  docker exec -i "${SPRINT12_DB_CONTAINER}" \
    psql -U postgres -d postgres -v ON_ERROR_STOP=1 \
    -f /tmp/sprint12_helpers.sql \
    -f /tmp/sprint12_gate_bc.sql
}

echo "=== Private publication gate pass 1 ==="
run_bc
echo "=== Private publication gate pass 2 (reset/replay) ==="
run_bc
echo "PRIVATE_PUBLICATION_GATE=PASS"
