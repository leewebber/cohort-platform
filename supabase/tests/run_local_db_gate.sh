...
docker cp "${TESTS_DIR}/sql/gate_v_apollo_week3_executable_protocols.sql" "${SPRINT12_DB_CONTAINER}:/tmp/sprint12_gate_v.sql"
docker cp "${TESTS_DIR}/sql/gate_w_apollo_week4_executable_protocols.sql" "${SPRINT12_DB_CONTAINER}:/tmp/sprint12_gate_w.sql"
docker cp "${TESTS_DIR}/sql/gate_x_apollo_week5_executable_protocols.sql" "${SPRINT12_DB_CONTAINER}:/tmp/sprint12_gate_x.sql"
docker cp "${TESTS_DIR}/sql/gate_y_apollo_week6_executable_protocols.sql" "${SPRINT12_DB_CONTAINER}:/tmp/sprint12_gate_y.sql"
docker cp "${TESTS_DIR}/sql/gate_z_apollo_week7_executable_protocols.sql" "${SPRINT12_DB_CONTAINER}:/tmp/sprint12_gate_z.sql"
docker cp "${TESTS_DIR}/sql/gate_aa_apollo_week8_executable_protocols.sql" "${SPRINT12_DB_CONTAINER}:/tmp/sprint12_gate_aa.sql"
docker cp "${TESTS_DIR}/sql/gate_ab_apollo_week9_executable_protocols.sql" "${SPRINT12_DB_CONTAINER}:/tmp/sprint12_gate_ab.sql"
...
docker cp "${TESTS_DIR}/sql/gate_ai_authenticated_assigned_protocol_reads.sql" "${SPRINT12_DB_CONTAINER}:/tmp/sprint12_gate_ai.sql"
docker cp "${TESTS_DIR}/sql/gate_aj_authenticated_assigned_session_start.sql" "${SPRINT12_DB_CONTAINER}:/tmp/sprint12_gate_aj.sql"
docker cp "${TESTS_DIR}/sql/gate_ak_apollo_structured_warmup.sql" "${SPRINT12_DB_CONTAINER}:/tmp/sprint12_gate_ak.sql"
...
docker cp "${TESTS_DIR}/sql/gate_bb_private_enrolment_and_authored_dates.sql" "${SPRINT12_DB_CONTAINER}:/tmp/sprint12_gate_bb.sql"
docker cp "${TESTS_DIR}/sql/gate_bc_private_publication_and_activation.sql" "${SPRINT12_DB_CONTAINER}:/tmp/sprint12_gate_bc.sql"
...
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -f /tmp/sprint12_gate_v.sql
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -f /tmp/sprint12_gate_w.sql
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -f /tmp/sprint12_gate_x.sql
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -f /tmp/sprint12_gate_y.sql
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -f /tmp/sprint12_gate_z.sql
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -f /tmp/sprint12_gate_aa.sql
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -f /tmp/sprint12_gate_ab.sql
...
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -f /tmp/sprint12_gate_ai.sql
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -f /tmp/sprint12_gate_aj.sql
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -f /tmp/sprint12_gate_ak.sql
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -f /tmp/sprint12_gate_am.sql
...
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -1 -f /tmp/sprint12_correction_migration.sql
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -1 -f /tmp/sprint12_interval_integrity_migration.sql
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -1 -f /tmp/sprint12_interval_correction_migration.sql
...
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -f /tmp/sprint12_gate_aw.sql
...
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -f /tmp/sprint12_gate_bb.sql
docker exec -i "${SPRINT12_DB_CONTAINER}" psql -U postgres -d postgres -v ON_ERROR_STOP=1 -f /tmp/sprint12_gate_bc.sql
