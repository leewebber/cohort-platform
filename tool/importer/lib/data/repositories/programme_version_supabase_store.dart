...
class ProgrammeVersionSupabaseStore implements ProgrammeVersionStore {
  ...
  Future<ProgrammeTemplateTree?> loadTemplateTree(String versionId) async {
    ...
    try {
      final phasesResponse = await SupabaseClientHolder.client
          .from(_phasesTable)
          .select()
          .eq('version_id', version.id)
          .order('phase_order', ascending: true);

      final weeksResponse = await SupabaseClientHolder.client
          .from(_weeksTable)
          .select()
          .eq('version_id', version.id)
  ...
  Future<void> saveTemplateTree({
    ...
    try {
      ...
      for (final phase in tree.template.phases) {
        ...
        final insertedPhase = await SupabaseClientHolder.client
            .from(_phasesTable)
            .insert(phasePayload)
            .select()
            .single();
      ...
      for (final weekNode in tree.weekNodes) {
        ...
        final insertedWeek = await SupabaseClientHolder.client
            .from(_weeksTable)
            .insert(weekPayload)
            .select()
            .single();
        ...
        importerLog('[ProgrammeTreeSave] inserted week id=$weekId');
        ...
        for (final dayNode in weekNode.sortedDays) {
          ...
          final insertedDay = await SupabaseClientHolder.client
              .from(_daysTable)
              .insert(dayPayload)
              .select()
              .single();
          ...
          for (final slot in dayNode.sortedSlots) {
            ...
            final insertedSlot = await SupabaseClientHolder.client
                .from(_slotsTable)
                .insert(slotPayload)
                .select()
                .single();
  ...
  Future<List<ProgrammeCatalogEntry>> listCatalogueVersions(
    ...
    try {
      var request = SupabaseClientHolder.client
          .from(_versionsTable)
  ...
  Future<ProgrammeLineage> insertLineage(ProgrammeLineage lineage) async {
    try {
      ...
      importerLog('[ProgrammeCreate] insertLineage payload=$payload');
  ...
  static void _logStoreFailure(
    ...
    importerLog('[ProgrammeCreate] store $stage exception=$error');
    importerLog('[ProgrammeCreate] stackTrace=$stackTrace');
  }
