#!/usr/bin/env bash
# Controlled Sprint 1.4B Flutter Self-Test 1 against Cohort Staging.
#
# Requires:
...
DOTENV_STAGING="${S14B_DOTENV_STAGING:-/tmp/s13b_dotenv_staging}"
DOTENV_BACKUP="${S14B_DOTENV_BACKUP:-/tmp/s13b_dotenv_prod_backup}"
CREDS_JSON="${S14B_ATHLETE_CREDS:-/tmp/s13b_athlete_creds.json}"
SECRETS_FILE="lib/s14b_staging_secrets.g.dart"
SECRETS_BACKUP="/tmp/s14b_secrets_stub_backup.dart"
REPORT_FILE="${S14B_FLUTTER_REPORT:-/tmp/s14b_flutter_staging_report.txt}"
DEVICE="${S14B_FLUTTER_DEVICE:-chrome}"
...
stub=Path('lib/s14b_staging_secrets.g.dart').read_text()
...
import json
from pathlib import Path
creds=json.loads(Path("/tmp/s13b_athlete_creds.json").read_text())
a=creds["a"]
b=creds["b"]
env=Path(".env").read_text()
assert "tsbadngzgvsyfqjupkng" in env
assert "service_role" not in env.lower()
Path("lib/s14b_staging_secrets.g.dart").write_text(
f'''/// Temporary staging overwrite — restore after verification.
class S14BStagingSecrets {{
  ...
  static const String athleteBEmail = {json.dumps(b["email"])};
  static const String athleteBPassword = {json.dumps(b["password"])};
  static const String athleteBId = {json.dumps(b["user_id"])};
}}
'''
)
...
echo "Running flutter Self-Test 1 on device=$DEVICE..."
rm -f "$REPORT_FILE"
set +e
flutter run -d "$DEVICE" -t lib/main_s14b_staging_verify.dart \
  2>&1 | tee "$REPORT_FILE" &
RUN_PID=$!

python3 - <<'PY'
import json, pathlib, re, time, sys
log=pathlib.Path("/tmp/s14b_flutter_staging_report.txt")
deadline=time.time()+300
line_pattern=re.compile(r"S14B_FLUTTER_ASSERTIONS_JSON (\{.*)$", re.M)
while time.time()<deadline:
    if log.exists():
        ...
        if matches:
            ...
            pathlib.Path("/tmp/s14b_flutter_assertions.json").write_text(json.dumps(data, indent=2))
            if fails or not data.get("ok"):
                sys.exit(1)
            print("FLUTTER_STAGING_SELF_TEST_1_OK", flush=True)
            sys.exit(0)
...
pkill -f 'main_s14b_staging_verify' 2>/dev/null || true
