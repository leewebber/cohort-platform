...
void main() {
  ...
  group('AdaptationMetadataCompletenessAnalyzer', () {
    test('flags missing session intent, min duration, and derived block defaults', () {
      ...
      expect(item.missingCategories, contains(
      ...
      expect(item.suggestedPrimarySessionIntent?.intent, SessionIntent.threshold);
      expect(item.confirmedPrimarySessionIntent, isNull);
    });

    test('session tagged only when canonical fields and explicit blocks present', () {
      final report = analyzer.buildReport([
        AdaptationMetadataCompletenessInput(
  ...
  group('Suggestion vs confirmed intent', () {
    ...
    test('confirmSuggestedIntent returns null without founder confirmation', () {
      const suggestion = SessionIntentSuggestion(
        intent: SessionIntent.threshold,
        reason: 'test',
      );
      expect(
        confirmSuggestedIntent(suggestion: suggestion, founderConfirmed: false),
        isNull,
      );
      expect(
        confirmSuggestedIntent(suggestion: suggestion, founderConfirmed: true),
