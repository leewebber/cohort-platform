...
void main() {
  group(
    'Protocol canonical adaptation metadata (shared Session Definition model)',
    () {
      ...
      test('persists and reloads through performance_protocols row maps', () {
        ...
        expect(
          upsert[SessionAdaptationMetadataKeys.primarySessionIntent],
          'threshold',
        );
        expect(upsert[SessionAdaptationMetadataKeys.secondarySessionIntents], [
          'aerobic_base',
        ]);
        expect(
          upsert[SessionAdaptationMetadataKeys.minimumViableDurationMin],
          28,
        );
      ...
      test(
        'legacy protocol with only primary_capability does not infer session intent',
        () {
      ...
      test('clearing adaptation fields writes nulls on upsert', () {
        ...
        expect(
          row[SessionAdaptationMetadataKeys.minimumViableDurationMin],
          isNull,
        );
      });
  ...
  group('Protocol duplication and programme insertion', () {
    ...
    test(
      'assign unchanged keeps metadata on referenced cohort protocol row',
      () {
    ...
    test(
      'prepareCopy then builder edit preserves edited canonical metadata',
      () async {
  ...
  group('Protocol Builder UI', () {
    testWidgets('cohort protocol admin shows shared adaptation controls', (
      tester,
    ) async {
