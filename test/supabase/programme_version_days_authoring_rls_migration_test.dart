...
void main() {
  ...
  group('programme_version_days authoring RLS fix', () {
    ...
    test(
      'replaces day-id policies with week_id ownership for INSERT RETURNING',
      () {
        expect(
          migrationSql,
    ...
    test('week helper joins lineage and version owner_id to auth.uid()', () {
      expect(
        migrationSql,
        contains('JOIN programme_lineages l ON l.id = v.lineage_id'),
      );
    ...
    test('session slot write policy uses week ownership chain', () {
      expect(
        migrationSql,
        contains(
          'DROP POLICY IF EXISTS programme_version_session_slots_write_coach',
        ),
      );
      expect(
        migrationSql,
        contains(
          'cohort_programme_week_is_dev_coach_draft_writable(d.week_id)',
        ),
      );
    });
