...
void main() {
  ...
  group('authenticated programme authoring INSERT RETURNING fix', () {
    ...
    test(
      'replaces indirect lineage select policy with direct created_by predicate',
      () {
        expect(
          migrationSql,
          contains('DROP POLICY IF EXISTS programme_lineages_select_coach'),
        );
        expect(
          migrationSql,
          contains(
    ...
    test(
      'replaces indirect version select policy with direct owner row predicates',
      () {
        expect(
          migrationSql,
          contains('DROP POLICY IF EXISTS programme_versions_select_coach'),
        );
        expect(
          migrationSql,
          contains(
    ...
    test('preserves insert ownership checks without broadening access', () {
      expect(
        migrationSql,
        isNot(
          contains('DROP POLICY IF EXISTS programme_lineages_insert_coach'),
        ),
      );
      expect(
        migrationSql,
        isNot(
          contains(
            'DROP POLICY IF EXISTS programme_versions_insert_coach_draft',
          ),
        ),
      );
    ...
    test('documents cross-coach denial verification', () {
      expect(
        migrationSql,
        contains(
          'Different authenticated coach SELECT/UPDATE those rows → denied',
        ),
      );
    });
  });
