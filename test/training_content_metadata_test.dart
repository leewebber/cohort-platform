...
void main() {
  group('TrainingContentKindDb', () {
    ...
    test('round-trips session', () {
      expect(TrainingContentKindDb.fromDb('session').dbValue, 'session');
    });
  ...
  group('ProtocolDraft metadata mapping', () {
    ...
    test(
      'editing ordinary fields does not reset default cohort classification',
      () {
        final draft = ProtocolDraft(
          protocolId: 'RN-006',
          name: 'Classic Threshold',
  ...
  group('TrainingContentClassification invariants', () {
    ...
    test('programme-only session requires programmeVersionId', () {
      ...
      expect(
        TrainingContentClassification.isProgrammeOnlySession(draft),
        isTrue,
      );
    });
    ...
    test('reusable coach session requires ownerId', () {
      ...
      expect(
        TrainingContentClassification.isReusableCoachSession(draft),
        isTrue,
      );
      expect(
  ...
  group('training content migration contract', () {
    ...
    test('adds metadata columns with cohort defaults', () {
      expect(
        migrationSql,
        contains("content_kind TEXT NOT NULL DEFAULT 'cohort_protocol'"),
      );
      expect(
        migrationSql,
        contains("authoring_scope TEXT NOT NULL DEFAULT 'cohort_global'"),
      );
      expect(
        migrationSql,
        contains("endorsement_status TEXT NOT NULL DEFAULT 'cohort_endorsed'"),
      );
    });
    ...
    test('defines check constraints', () {
      expect(
        migrationSql,
        contains('performance_protocols_content_kind_check'),
      );
      expect(
        migrationSql,
        contains('performance_protocols_authoring_scope_check'),
      );
      expect(
        migrationSql,
        contains('performance_protocols_endorsement_status_check'),
      );
      expect(
        migrationSql,
        contains('performance_protocols_source_content_kind_check'),
      );
    });
    ...
    test('backfills legacy cohort protocols and logs count', () {
      ...
      expect(
        migrationSql,
        contains('[TrainingContentMigration] legacy cohort protocol count='),
      );
    });
    ...
    test('FK programme_version_id references programme_versions', () {
      expect(
        migrationSql,
        contains('performance_protocols_programme_version_id_fkey'),
      );
