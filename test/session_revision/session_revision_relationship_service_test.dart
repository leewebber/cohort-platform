...
void main() {
  ...
  group('direct programme references', () {
    ...
    test(
      'same revision used in multiple slots in one programme version',
      () async {
        final lineage = SessionRevisionUsageTestFixtures.seedLineage(
          programmeTables,
        );
  ...
  group('active assignment references', () {
    ...
    test(
      'active assignment to referencing programme version is reported',
      () async {
        final lineage = SessionRevisionUsageTestFixtures.seedLineage(
          programmeTables,
        );
    ...
    test(
      'assignment to non-referencing programme version is not reported',
      () async {
        final lineage = SessionRevisionUsageTestFixtures.seedLineage(
          programmeTables,
        );
    ...
    test('multiple active assignments deduplicate by assignment id', () async {
      ...
      expect(summary.activeAssignmentReferences, hasLength(1));
      expect(
        summary.activeAssignmentReferences.first.assignmentId,
        'assignment-1',
      );
    });

    test(
      'assignment remains linked to old revision after newer programme version exists',
      () async {
        final lineage = SessionRevisionUsageTestFixtures.seedLineage(
          programmeTables,
        );
        ...
        expect(
          summaryV1.activeAssignmentReferences.first.programmeVersionId,
          versionOne.id,
        );
  ...
  group('historical usage', () {
    test(
      'terminal record with matching source_protocol_id is counted',
      () async {
    ...
    test(
      'archived revision metadata does not affect historical count',
      () async {
  ...
  group('combined summary', () {
    test(
      'classifications cover direct operational historical unused and all three',
      () async {
        final lineage = SessionRevisionUsageTestFixtures.seedLineage(
          programmeTables,
        );
        ...
        expect(
          historicalOnly.classifications,
          containsAll([
            ContentUsageClassification.directAuthored,
            ContentUsageClassification.historicalPerformance,
          ]),
    ...
    test(
      'stable result ordering and no duplicate assignment references',
      () async {
        final lineage = SessionRevisionUsageTestFixtures.seedLineage(
          programmeTables,
          code: 'PROG-B',
        ...
        expect(
          summary.programmeReferences.first.programmeLineageCode,
          'PROG-A',
        );
        ...
        expect(
          summary.activeAssignmentReferences
